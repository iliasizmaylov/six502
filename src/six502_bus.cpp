...
result_t
BUS_six502::broadcast_read(const addr_t& addr, databus_t& data)
{
...
result_t
BUS_six502::broadcast_write(const addr_t& addr, const databus_t& data)
{
    result_t res = SIX502_RET_STRAY_MEM;
...
DEV_six502 *BUS_six502::get_device_at_addr(const addr_t& addr)
{
    databus_t buf;
...
result_t BUS_six502::load_from_file_64(const std::string& path)
{
    databus_t buf;
    char buf_c;
    addr_t addr = 0x0000;
...
result_t BUS_six502::fetch_device_data(addr_range_t& range,
...
result_t BUS_six502::fetch_instructions(addr_t start,
        struct instruction_ctx *out, const u16& count, u16& num)
{
    databus_t buf;
    num = 0;
